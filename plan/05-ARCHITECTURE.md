...
| Code editor | **Custom** (~100 lines): textarea over a syntax-highlighted `<pre>` | Monaco is ~5 MB and CDN-loaded by default, which breaks the offline requirement, and fights the design system. A small C-family tokenizer covers everything a 20-minute debugging round needs. |
